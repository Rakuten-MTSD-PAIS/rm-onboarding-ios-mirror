...
public class JPKIIDSDKAdapter: JPKIProtocol {
    ...
    // eKYC configuration URLs
    private var jpkiUrl: String = "https://stg-jpki.id.rakuten.co.jp"
    private var languageUrl: String = "https://stg-qa.static.id.rakuten.co.jp/static/ekyc/ja/generic.json"
    ...
    public func triggerJPKI(completion: @escaping (Result<(easyId: String, exchangeToken: String), Error>) -> Void) {
        ...
        sessionProvider.session(mediation: mediationOptions) { [weak self] result in
            ...
            case .success(let session):
                ...
                self?.proceedWithEKYC(session: session, sessionProvider: sessionProvider, navigationController: navigationController, completion: completion)
    ...
    private func proceedWithEKYC(
        ...
        completion: @escaping (Result<(easyId: String, exchangeToken: String), Error>) -> Void
    ) {
        ...
        do {
            ekycConfig = try EKycOptionsBuilder()
                ...
                .set(redeemerClientId: "rmn_app_ios")
                .build()
        } catch {
        ...
        sessionProvider.eKYC(configuration: ekycConfig) { result in
            ...
            case .success(let token):
                ...
                // Return both easyId and exchangeToken
                completion(.success((easyId: easyId, exchangeToken: token.value)))
